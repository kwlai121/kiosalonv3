...
if ( !class_exists( 'ReduxFramework_wbc_importer' ) ) {
    ...
    class ReduxFramework_wbc_importer {
        ...
        public function render() {
			...
			if (!$code) {
				echo "<h4>".esc_html__( 'Please verify purchase code before import demo data.', LION_CURRENT_THEME )."</h4>";
			} else {
				if ( !empty( $this->field['wbc_demo_imports'] ) ) {
					...
					foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
						...
						if ( !array_key_exists( 'imported', $imports ) ) {
							...
							$imported = false;
							$import_message = esc_html__( 'Import Demo', LION_CURRENT_THEME );
						}else {
							$imported = true;
							$extra_class = 'active imported';
							$import_message = esc_html__( 'Demo Imported', LION_CURRENT_THEME );
						}
						...
						if ( false == $imported ) {
							echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', LION_CURRENT_THEME ).'</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', LION_CURRENT_THEME ) . '</span></div>';
						}else {
							echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', LION_CURRENT_THEME ).'</div>';
							echo '<span class="spinner">'.esc_html__( 'Please Wait...', LION_CURRENT_THEME ).'</span>';
							echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', LION_CURRENT_THEME ).'</div>';
						}
						echo '</div>';
						echo '</div>';
					...
					echo "<h5>".esc_html__( 'No Demo Data Provided', LION_CURRENT_THEME )."</h5>";
				}
			}
            echo '</div></div>';
            echo '</fieldset></td></tr>';
